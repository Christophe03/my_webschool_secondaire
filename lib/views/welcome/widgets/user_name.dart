...
class UserName extends StatelessWidget {
  ...
  Future<String> checkPhoto() async {
    String path = "";
    await Future.delayed(const Duration(seconds: 0)); // Simuler un délai
    Directory appDocumentsDirectory =
        await getApplicationDocumentsDirectory(); // Récupérer le répertoire des documents de l'application
    path = appDocumentsDirectory.path;
    bool pfExists = await File('$path/profil.png').exists();
    if (pfExists) {
      return '$path/profil.png'; // Retourner le chemin du fichier si trouvé
    } else {
        ...
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            if (snapshot.hasError) {
              return Container(
                height: 41,
                alignment: Alignment.center, // Centrer l'image en cas d'erreur
                child: Image.asset(
                  'assets/images/profil.png',
                  fit: BoxFit.contain,
                  width: 41,
              ...
              child: Row(
                children: [
                  snapshot.data != ''
                      ? Container(
                          height: 41,
                          alignment: Alignment.center, // Centrer l'avatar
                          child: CircleAvatar(
                            radius: 32,
